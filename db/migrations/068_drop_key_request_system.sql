-- 068: Remove the user-facing key request workflow.
--
-- The product no longer wants users to request keys through the app — they email
-- the facility office directly. Key INVENTORY (the keys/key_assignments/
-- key_orders/key_stock_transactions/key_audit_logs tables) is kept intact for
-- internal stock + issuance tracking.
--
-- key_requests had 0 rows, so this is data-loss-free.
--
-- Steps:
-- 1. Recreate admin_delete_user, delete_room_cascade, safely_delete_key without
--    their key_requests references (same pattern used in 067 for maintenance_requests).
-- 2. Drop the FK column key_orders.request_id (all values are NULL — verified).
-- 3. Drop the key_requests table (cascades 5 triggers + 4 RLS policies).
-- 4. Drop the orphaned trigger functions that only existed for key_requests:
--    auto_create_key_order, notify_admins_of_key_request, trg_emit_key_request_updates.

-- ---------------------------------------------------------------------------
-- 1. Cascade fns — drop the key_requests lines
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  WHERE profile_id = p_user_id OR occupant_id IN (SELECT id FROM public.occupants WHERE profile_id = p_user_id);

  DELETE FROM public.key_order_items WHERE order_id IN (
    SELECT id FROM public.key_orders
    WHERE requestor_id = p_user_id OR user_id = p_user_id OR ordered_by = p_user_id
       OR received_by = p_user_id OR delivered_by = p_user_id
       OR recipient_id IN (SELECT id FROM public.occupants WHERE profile_id = p_user_id));
  DELETE FROM public.key_stock_transactions WHERE order_id IN (
    SELECT id FROM public.key_orders
    WHERE requestor_id = p_user_id OR user_id = p_user_id OR ordered_by = p_user_id
       OR received_by = p_user_id OR delivered_by = p_user_id
       OR recipient_id IN (SELECT id FROM public.occupants WHERE profile_id = p_user_id));
  ...
  WHERE occupant_id IN (SELECT id FROM public.occupants WHERE profile_id = p_user_id) OR profile_id = p_user_id;
...
BEGIN
  SELECT EXISTS (SELECT 1 FROM public.user_roles
    WHERE user_id = _caller_id AND role::text IN ('admin', 'facilities_manager')) INTO _is_allowed;
  IF NOT _is_allowed THEN RAISE EXCEPTION 'Permission denied – requires admin or facilities_manager role'; END IF;
  ...
  DELETE FROM public.clerk_assignments WHERE room_id = p_room_id;
  DELETE FROM public.court_activity_log WHERE from_room_id = p_room_id OR to_room_id = p_room_id;
  ...
  DELETE FROM public.inventory_audits WHERE storage_room_id = p_room_id;
  DELETE FROM public.inventory_items WHERE storage_room_id = p_room_id;
  DELETE FROM public.room_relocations WHERE original_room_id = p_room_id OR temporary_room_id = p_room_id;
  DELETE FROM public.staff_tasks WHERE to_room_id = p_room_id OR from_room_id = p_room_id;
  DELETE FROM public.issues WHERE room_id = p_room_id;
  DELETE FROM public.room_properties WHERE space_id = p_room_id OR parent_room_id = p_room_id;
  DELETE FROM public.unified_spaces WHERE parent_room_id = p_room_id;
  DELETE FROM public.rooms WHERE parent_room_id = p_room_id;
  DELETE FROM public.rooms WHERE id = p_room_id;
  DELETE FROM public.unified_spaces WHERE id = p_room_id;
END;
$function$;

CREATE OR REPLACE FUNCTION public.safely_delete_key(key_id_to_delete uuid)
 RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = auth.uid() AND role = 'admin') THEN
    RAISE EXCEPTION 'Only administrators can delete keys';
  END IF;

  DELETE FROM public.key_audit_logs WHERE key_id = key_id_to_delete;
  DELETE FROM public.key_stock_transactions WHERE key_id = key_id_to_delete;
  DELETE FROM public.office_elevator_card_allocations WHERE key_id = key_id_to_delete;
  DELETE FROM public.key_assignments WHERE key_id = key_id_to_delete;
  DELETE FROM public.key_order_items WHERE order_id IN (SELECT id FROM public.key_orders WHERE key_id = key_id_to_delete);
  DELETE FROM public.key_orders WHERE key_id = key_id_to_delete;
  UPDATE public.lockbox_slots SET key_id = NULL, status = 'empty' WHERE key_id = key_id_to_delete;
  DELETE FROM public.keys WHERE id = key_id_to_delete;
END;
$function$;

-- ---------------------------------------------------------------------------
-- 2. Drop the FK column on key_orders (all rows are NULL, verified pre-flight).
-- ---------------------------------------------------------------------------
ALTER TABLE public.key_orders DROP COLUMN IF EXISTS request_id;

-- ---------------------------------------------------------------------------
-- 3. Drop the table (cascades its 5 triggers + 4 RLS policies).
-- ---------------------------------------------------------------------------
DROP TABLE IF EXISTS public.key_requests CASCADE;

-- ---------------------------------------------------------------------------
-- 4. Drop trigger functions that only existed for key_requests.
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.auto_create_key_order();
DROP FUNCTION IF EXISTS public.notify_admins_of_key_request();
