-- 067: Drop the orphaned maintenance_requests table.
--
-- maintenance_requests had no readers and both writers inserted columns that did
-- not exist on it; it has always held 0 rows. The live maintenance feature uses
-- maintenance_schedules + issues, and the form processors now create real issues
-- (see migration 066 / commit). The only remaining DB reference was two
-- nullify-on-user-delete statements in admin_delete_user, removed below before the
-- drop so user deletion keeps working.

-- Recreate admin_delete_user without the two maintenance_requests UPDATEs.
...
-- Now safe to drop the orphan table (RLS policies drop with it).
