-- Migration: 115_fix_admin_delete_user_remaining_fk_gaps.sql
-- admin_delete_user (065) covers most FK chains to profiles(id)/auth.users(id)
-- explicitly, but buildings.created_by, rooms.created_by/updated_by, and
-- title_access_rules.created_by (all REFERENCES auth.users(id) with no
-- ON DELETE clause, i.e. default NO ACTION) were never added to that list.
-- Deleting a user who has ever created/edited a room or building fails with
-- a foreign key violation. All four columns are nullable, so SET NULL is safe.
...
BEGIN
  ...
  UPDATE public.buildings SET created_by = NULL WHERE created_by = p_user_id;
  ...
  UPDATE public.rooms SET updated_by = NULL WHERE updated_by = p_user_id;
  ...
  UPDATE public.title_access_rules SET created_by = NULL WHERE created_by = p_user_id;
