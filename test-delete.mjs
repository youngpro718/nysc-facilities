...
async function testDelete() {
  ...
  if (courtRoom) {
      ...
      console.log('  room_shutdowns err:', res1.error);
      const res2 = await supabase.from("court_attendance").delete().eq("room_id", id);
      console.log('  court_attendance err:', res2.error);
  } else {
  ...
  const { error: roomError, data } = await supabase
    .from("rooms")
    .delete()
    .eq("id", id)
    .select();

  console.log('Room delete result:', { roomError, data });
}
testDelete();
