-- Fix the 6 specific Security Definer Views identified by the linter
...
FROM court_rooms cr
LEFT JOIN maintenance_schedules ms ON cr.room_number = ms.space_name;

-- 4. Fix public.key_inventory_view view
...
SELECT 
    ...
    k.updated_at,
    CASE 
        WHEN k.available_quantity <= k.minimum_stock THEN 'low_stock'
...
-- 6. Fix public.user_verification_view view (already fixed but ensure it's correct)
