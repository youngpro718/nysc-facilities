-- Fix remaining Security Definer Views with correct enum values
...
-- Recreate spaces view without SECURITY DEFINER (fix type casting)
...
LEFT JOIN maintenance_schedules ms ON ms.space_name = cr.room_number 
    AND ms.space_type = 'room';

-- Recreate key_inventory_view without SECURITY DEFINER (remove WHERE clause for now)
...
-- Recreate user_verification_view without SECURITY DEFINER
