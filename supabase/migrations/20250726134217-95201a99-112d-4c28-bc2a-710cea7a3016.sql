...
-- Step 1: Drop all existing conflicting RLS policies on user_roles
...
-- Step 2: Create security definer function to safely check user roles
...
-- Step 3: Create security definer function to check if user has specific role
...
-- Step 4: Create security definer function to check if user is admin
...
-- Step 5: Create new, safe RLS policies for user_roles
...
-- Step 6: Create secure role management functions
...
-- Step 7: Fix search_path in existing functions
CREATE OR REPLACE FUNCTION public.validate_role_assignment(user_id uuid, role user_role)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
-- Step 8: Create security audit table for monitoring
...
-- Step 9: Create function to log security events
...
-- Step 10: Add input validation functions
...
-- Step 11: Create rate limiting table
...
-- Step 12: Create rate limiting function
...
-- Step 13: Update existing functions to use proper search_path
...
-- Step 14: Grant proper permissions
