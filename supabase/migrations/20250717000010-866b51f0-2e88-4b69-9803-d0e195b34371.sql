-- Phase 1: Database Schema Fixes for Key Management System (Handle Dependencies)

-- First, add missing columns to key_orders table (except status which we'll handle separately)
...
-- Handle status column conversion for key_requests (safer approach)
DO $$ 
DECLARE
    col_exists boolean;
BEGIN
    -- Check if the status column is already the correct type
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'key_requests' 
        AND column_name = 'status' 
        AND udt_name = 'key_request_status'
    ) INTO col_exists;
    
    -- Only convert if not already correct type
    IF NOT col_exists THEN
        -- Add a temporary column with the new enum type
        ALTER TABLE key_requests ADD COLUMN status_temp key_request_status;
        
        -- Update the new column based on existing values
        UPDATE key_requests SET status_temp = 
            CASE 
        ...
        ALTER TABLE key_requests RENAME COLUMN status_temp TO status;
...
-- Handle status column conversion for key_orders (drop dependent views first)
DO $$ 
DECLARE
    col_exists boolean;
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'key_orders' 
        AND column_name = 'status' 
        AND udt_name = 'key_order_status'
    ) INTO col_exists;
    ...
    IF NOT col_exists THEN
        ...
        -- Add a temporary column with the new enum type
        ALTER TABLE key_orders ADD COLUMN status_temp key_order_status;
        
        -- Update the new column based on existing values
        UPDATE key_orders SET status_temp = 
            CASE 
        ...
        ALTER TABLE key_orders RENAME COLUMN status_temp TO status;
        ...
        -- Recreate the view if it existed (basic version)
        CREATE VIEW key_orders_view AS
        SELECT 
            ko.*,
            kr.reason,
            kr.request_type,
