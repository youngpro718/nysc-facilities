...
-- Update the fulfillment_stage enum to be simpler
DROP TYPE IF EXISTS fulfillment_stage_enum CASCADE;
CREATE TYPE fulfillment_stage_enum AS ENUM ('pending', 'in_progress', 'completed');

-- Update the fulfillment_stage column to use the new enum
ALTER TABLE supply_requests 
ALTER COLUMN fulfillment_stage TYPE fulfillment_stage_enum 
USING CASE 
  WHEN fulfillment_stage IN ('assigned', 'picking', 'picked', 'packing', 'packed', 'ready_for_delivery') THEN 'in_progress'::fulfillment_stage_enum
  WHEN fulfillment_stage = 'completed' THEN 'completed'::fulfillment_stage_enum
  ELSE 'pending'::fulfillment_stage_enum
END;

-- Set default for fulfillment_stage
ALTER TABLE supply_requests 
ALTER COLUMN fulfillment_stage SET DEFAULT 'pending'::fulfillment_stage_enum;
