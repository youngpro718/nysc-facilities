...
-- 1. First, let's create the missing security validation functions that are referenced in the code
...
-- 2. Update existing functions to have proper search_path (fixing WARN issues)
-- Note: We'll update the most critical ones that don't already have SET search_path
...
-- 3. Remove security definer views and replace with regular views
-- First, let's drop the problematic security definer views if they exist
...
-- Create replacement views without security definer
...
JOIN profiles p ON ka.assigned_to = p.id
WHERE k.type = 'elevator_pass'
  AND ka.status IN ('assigned', 'returned');

-- Enable RLS on the base tables to maintain security
ALTER TABLE key_assignments ENABLE ROW LEVEL SECURITY;
ALTER TABLE keys ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for key_assignments if not exists
DO $$
BEGIN
  IF NOT EXISTS (
