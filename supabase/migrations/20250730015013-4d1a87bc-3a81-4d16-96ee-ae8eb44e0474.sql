-- Drop existing functions before recreating them with security fixes
DROP FUNCTION IF EXISTS public.get_court_personnel();
DROP FUNCTION IF EXISTS public.get_building_hierarchy();
DROP FUNCTION IF EXISTS public.get_energy_analytics();
DROP FUNCTION IF EXISTS public.get_facility_analytics();
DROP FUNCTION IF EXISTS public.get_spaces_dashboard_data(UUID, TEXT);

-- Recreate get_court_personnel function with secure search_path
...
-- Recreate get_building_hierarchy function with secure search_path
...
-- Recreate get_energy_analytics function with secure search_path
...
BEGIN
    ...
    SELECT 
        b.id,
        b.name as building_name,
        COALESCE(0::NUMERIC, 0) as total_consumption,
        COALESCE(0::NUMERIC, 0) as average_consumption,
        COALESCE(85::NUMERIC, 0) as efficiency_score,
        now() as last_reading
    FROM buildings b;
END;
$$;

-- Recreate get_facility_analytics function with secure search_path
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE((SELECT COUNT(*) FROM maintenance_requests WHERE status = 'pending'), 0) as maintenance_requests,
        CASE 
...
-- Recreate get_spaces_dashboard_data function with secure search_path
