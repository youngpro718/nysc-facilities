-- Corrected Safe Analytics Database Fixes
...
SELECT 
    ...
    -- Occupancy metrics (using room assignments with correct column)
...
LEFT JOIN (
    -- Occupancy subquery - use expiration_date to determine active assignments
    SELECT 
        room_id,
...
-- 5. Create energy analytics function (safe version with error handling)
...
DECLARE
    result JSON;
    lighting_count integer := 0;
    efficient_count integer := 0;
BEGIN
    -- Try to get lighting data, handle if table doesn't exist
    BEGIN
        SELECT 
            COALESCE(COUNT(*), 0),
...
CREATE INDEX IF NOT EXISTS idx_occupant_room_assignments_room_expiration ON occupant_room_assignments(room_id, expiration_date);
