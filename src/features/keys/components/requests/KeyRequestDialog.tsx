...
export function KeyRequestDialog({ open, onOpenChange, onSuccess }: KeyRequestDialogProps) {
  ...
  const [requestType, setRequestType] = useState<'new' | 'spare' | 'replacement' | 'temporary'>('new');
      ...
      <div className="space-y-5">
        ...
        {rooms.length > 0 && (
          <div className="space-y-2">
            ...
            <Select value={roomId || '__other__'} onValueChange={(value) => setRoomId(value === '__other__' ? '' : value)}>
              <SelectTrigger id="key-request-room" aria-label="Assigned room"><SelectValue placeholder="Choose a room" /></SelectTrigger>
              <SelectContent>
                {rooms.map((room) => (
        ...
        <div className="flex justify-end gap-2 border-t pt-4">
          ...
          <Button type="button" onClick={handleSubmit} disabled={submitting || keyRequestsUnavailable}>
