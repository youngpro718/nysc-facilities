  ...
  const editSpaceMutation = useMutation({
    mutationFn: async (data: RoomFormData) => {
      ...
      if (dbData.room_type === RoomTypeEnum.COURTROOM) {
        try {
          await storageService.ensureBucketsExist([STORAGE_BUCKETS.courtroomPhotos]);
          if (data.courtroom_photos && roomId) {
            ...
            if (allPhotos.length > 0) {
              await storageService.cleanupOrphanedFiles(STORAGE_BUCKETS.courtroomPhotos, roomId, allPhotos);
            }
          }
        } catch {
