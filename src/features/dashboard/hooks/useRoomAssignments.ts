  ...
  const { data: assignedRooms = [] } = useQuery<UserAssignment[]>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (roomsError) {
          logger.warn('Error fetching room details:', roomsError);
          return [];
        }
        ...
        if (floorsError) {
          logger.warn('Error fetching floor details:', floorsError);
          return [];
        }
        ...
        if (buildingsError) {
          logger.warn('Error fetching building details:', buildingsError);
          return [];
        }
