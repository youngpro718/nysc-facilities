...
export function useCourtOperationsRealtime() {
  ...
  useEffect(() => {
    const broadcastChannel = supabase.channel("court_ops_broadcast").subscribe();
  ...
  const onMoveJudge = async (
    ...
    try {
      ...
      if (broadcastRef.current) {
        await broadcastRef.current.send({ type: "broadcast", event: "judge_move_pending", payload: { fromRoomId, toRoomId, judgeName, isCovering } });
      }
      ...
      logger.error("onMoveJudge error", error);
      throw error;
    }
  };
