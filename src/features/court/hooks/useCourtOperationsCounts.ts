...
export function useCourtOperationsCounts(): CourtOperationsCounts {
  ...
  const { data: todaysCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        logger.warn('Error counting today sessions:', error);
        return 0;
      }

      return count || 0;
    },
    staleTime: QUERY_CONFIG.stale.short, // 1 minute
  });
  ...
  const { data: dailyCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        logger.warn('Error counting daily sessions:', error);
        return 0;
      }

      return count || 0;
    },
    staleTime: QUERY_CONFIG.stale.short,
  });
  ...
  const { data: assignmentsData = { needAttention: 0, details: '' } } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        logger.warn('Error fetching assignments:', error);
  ...
  const { data: maintenanceCount = 0, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        logger.warn('Error counting maintenance issues:', error);
        return 0;
      }
