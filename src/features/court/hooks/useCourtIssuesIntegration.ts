// Court Issues Integration — realtime courtroom issue tracking
...
export const useCourtIssuesIntegration = () => {
  ...
  const { data: buildingIssues = [], isLoading: buildingIssuesLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('issues')
        .select('id, title, description, status, priority, building_id, floor_id, location_description, created_at, updated_at, space_id, room_id')
        .in('status', ['open', 'in_progress'])
        .is('room_id', null)
        .order('priority', { ascending: false })
        .order('created_at', { ascending: false });
