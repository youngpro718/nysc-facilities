...
export function useSecureAuth() {
  ...
  const getRemainingAttempts = useCallback(async (email: string) => {
    try {
      ...
      const status = data[0];
      const maxAttempts = RETRY.loginMaxAttempts;
  ...
  const secureSignUp = useCallback(async (email: string, password: string, userData: Record<string, unknown>) => {
    ...
    try {
      ...
      // Use simplified password validation (fallback to client-side if RPC unavailable)
      ...
      try {
        ...
        if (!passwordError && passwordValidation) {
          const pwdResult = passwordValidation as { is_valid: boolean; errors: string[] };
          passwordIsValid = pwdResult.is_valid;
          passwordErrors = pwdResult.errors || [];
        }
      } catch {
        // RPC unavailable — client-side fallback already set above
      }
      if (!passwordIsValid) {
