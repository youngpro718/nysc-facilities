...
- Marking an order ready + deducting inventory goes through the atomic `fulfill_supply_request(uuid, jsonb, text)` RPC (db/migrations/096_fix_supply_fulfillment.sql) directly — it does the status-gate check, item updates, inventory deduction, and transaction logging in one SECURITY DEFINER transaction. The three live fulfillment paths (`PartialFulfillmentDialog`, `ImprovedSupplyStaffDashboard` Quick Ready, `SupplyFulfillmentPanel`) call it directly; there is no service-layer wrapper.
