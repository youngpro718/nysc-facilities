...
export function useSupplyPendingCounts() {
  return useQuery({
    ...
    queryFn: async (): Promise<SupplyPendingCounts> => {
      ...
      if (approvalsError) {
        logger.warn('Error fetching pending approvals count:', approvalsError);
      }
      ...
      if (ordersError) {
        logger.warn('Error fetching pending orders count:', ordersError);
      }
      ...
      if (readyError) {
        logger.warn('Error fetching ready count:', readyError);
      }
