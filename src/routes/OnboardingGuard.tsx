  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      try {
        ...
        const publicPaths = ['/auth/', '/onboarding/', '/login', '/forms/', '/verification-pending'];
        ...
        //     supabase.auth.mfa.listFactors(), TIMEOUTS.mfaCheck, 'checking MFA factors');
        //   if (!factorError) {
        //     const hasVerifiedTotp = factorData?.totp?.some((f: any) => f.status === 'verified');
        //     if (!hasVerifiedTotp) { navigate('/auth/mfa', { replace: true }); return; }
        //   }
        // }
