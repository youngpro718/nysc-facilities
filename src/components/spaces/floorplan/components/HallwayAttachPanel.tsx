...
import { Link2, Unlink, Plus, ArrowUpDown, Building2 } from 'lucide-react';
    ...
    return (
      <div className="space-y-3">
        ...
        {attachedRooms.length === 0 ? (
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {attachedRooms.map((ar) => (
              <div key={ar.id} className="p-2 rounded-lg border bg-card text-card-foreground space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">{ar.room_number || ar.room_name || ar.room_id.slice(0, 8)}</span>
                  <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => handleDetach(ar.id)}>
                ...
                <div className="flex gap-2">
                  <div className="flex-1">
                    ...
                    <Select value={ar.side} onValueChange={(v) => handleUpdateAttachment(ar.id, { side: v })}>
                  ...
                  <div className="flex-1">
                    ...
                    <Select value={ar.position} onValueChange={(v) => handleUpdateAttachment(ar.id, { position: v })}>
                      <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="w-16">
                    ...
                    <Input
                      ...
                      value={ar.sequence_order}
                      onChange={(e) => handleUpdateAttachment(ar.id, { sequence_order: parseInt(e.target.value) || 0 })}
                    />
                  </div>
                </div>
              </div>
        ...
        <p className="text-xs text-muted-foreground flex items-center gap-1">
          ...
          Select a room in the 3D view, then attach it here.
        </p>
        {onEnterAttachMode && (
