...
import { CheckCircle, AlertTriangle, Edit2, Trash2, Save, X } from 'lucide-react';
              ...
              {sessions.map((session, index) => (
                <TableRow key={index} className={session.confidence < 0.8 ? 'bg-amber-50' : ''}>
                  ...
                  <TableCell>
                    {editingIndex === index ? (
                      <Input
                        value={editData.clerk_name || ''}
                        onChange={(e) => setEditData({ ...editData, clerk_name: e.target.value })}
                        className="w-20"
                      />
                    ) : (
                      session.clerk_name
