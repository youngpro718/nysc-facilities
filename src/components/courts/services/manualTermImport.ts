...
export async function createTermFromManualInput(text: string): Promise<TermImportData> {
  ...
  for (let i = 0; i < Math.min(3, lines.length); i++) {
    ...
    if (dateMatch) {
      // Convert to ISO format dates (YYYY-MM-DD)
      try {
        const startDate = new Date(dateMatch[1]);
        const endDate = new Date(dateMatch[2]);
        
        term.start_date = startDate.toISOString().split('T')[0];
        term.end_date = endDate.toISOString().split('T')[0];
      } catch (e) {
        console.warn('Could not parse dates:', dateMatch[1], dateMatch[2]);
      }
