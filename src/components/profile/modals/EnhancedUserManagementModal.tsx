...
export function EnhancedUserManagementModal({ open, onOpenChange, initialTab = 'all' }: UserManagementModalProps) {
  ...
  const handleVerifyUser = async (userId: string) => {
    try {
      ...
      // Map requested access to role
      const role = requested === 'administrative' || requested === 'admin' 
        ? 'admin' 
        : 'standard';

      // Use the new approve_user_verification function
      ...
      toast({
        title: "User Verified",
        description: `User has been successfully verified with ${role} role`
      });
  ...
  const handleRejectUser = async (userId: string) => {
    try {
      // Use the new reject_user_verification function
      const { data, error } = await supabase
