...
import { STORAGE_BUCKETS, TIMEOUTS } from '@/config';
...
export const storageService = {
    ...
    try {
      ...
      const { data, error: uploadError } = await supabase.storage
        ...
        .upload(filePath, file, {
          ...
          ...(options.metadata ? { cacheControl: String(TIMEOUTS.cacheControlHeader), metadata: options.metadata } : {})
        });
  ...
  async checkBucketExists(bucketName: string): Promise<boolean> {
    try {
      ...
      if (error) {
        logger.warn(`Error listing buckets: ${error.message}`, error);
        return false;
      }
  ...
  async getRoomPhotos(roomId: string): Promise<string[] | null> {
    return this.getEntityFiles(STORAGE_BUCKETS.roomPhotos, roomId);
  },
  ...
  async deleteRoomPhoto(roomId: string, photoUrl: string): Promise<boolean> {
    ...
    return this.removeFile(STORAGE_BUCKETS.roomPhotos, filename);
  }
};
...
export async function initializeStorage(): Promise<void> {
  ...
  await storageService.ensureBucketsExist([STORAGE_BUCKETS.courtroomPhotos, STORAGE_BUCKETS.termPdfs, STORAGE_BUCKETS.roomPhotos]);
}
