...
async function fetchRequestData(supabase: ReturnType<typeof createClient>, requestId: string) {
  const { data: request, error: requestError } = await supabase
    ...
    .select(
      "id, display_id, title, status, priority, created_at, approved_at, ready_for_delivery_at, fulfilled_at, delivery_location, justification, description, fulfilled_by"
    )
    .eq("id", requestId)
    .single();
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!body?.requestId || !["receipt", "fulfilled", "new_request_team"].includes(body.type)) {
      return new Response(
        JSON.stringify({ error: "Invalid payload: type and requestId required" }),
    ...
    console.error("send-supply-email error:", error);
    return new Response(
